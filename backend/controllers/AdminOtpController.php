...
class AdminOtpController
{
    ...
    public function adminApiLogin(): never
    {
        ...
        $this->pdo->prepare('UPDATE admins SET last_login_at = NOW() WHERE id = ?')->execute([(int)$admin['id']]);
    ...
    public function providersCreate(): void
    {
        ...
        $types = ['twilio', 'vonage', 'http_rest', 'test'];
        ...
        if (!empty($data['is_default'])) {
            $this->pdo->prepare('UPDATE otp_providers SET is_default = 0, updated_at = NOW() WHERE type = ?')->execute([$type]);
        }
    ...
    public function providersUpdate(int $id): void
    {
        ...
        if (!empty($data['is_default'])) {
            $this->pdo->prepare('UPDATE otp_providers SET is_default = 0, updated_at = NOW() WHERE type = ? AND id != ?')->execute([$type, $id]);
        }
